...
class WeddingViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action in ["public", "by_slug", "by_code"]:
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = WeddingCreateSerializer(
            data=request.data, context={"request": request}
        )
        serializer.is_valid(raise_exception=True)
        wedding = serializer.save(owner=request.user)
        return Response(
            WeddingSerializer(wedding).data, status=status.HTTP_201_CREATED
        )
    ...
    def current(self, request):
        ...
        wedding = (
            self.get_queryset().filter(status__in=["planning", "active"]).first()
        )
        if not wedding:
            return Response(
        ...
        return Response(self.get_serializer(wedding).data)
    ...
    def dashboard(self, request, pk=None):
        ...
        plus_ones = guests.filter(is_plus_one_coming=True).count()

        # Use a single query for children count instead of N+1
        children_count = Child.objects.filter(
            guest__in=guests.filter(has_children=True)
        ).count()

        tables_count = Table.objects.filter(wedding=wedding).count()
