...
class ChildViews(viewsets.ModelViewSet):
    ...
    def last_name_search(self, request):
        ...
        if not last_name:
            return Response({'message': 'provide last_name query param'}, status=status.HTTP_400_BAD_REQUEST)
        search = self.queryset.filter(last_name__icontains=last_name)
        if not search.exists():
            return Response({'message': 'last name not found'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def gender(self, request):
        ...
        if not gender:
            return Response({'message': 'provide gender query param'}, status=status.HTTP_400_BAD_REQUEST)
        search = self.queryset.filter(gender__icontains=gender)
        if not search.exists():
            return Response({'message': 'gender not found; valid: male, female, prefer_not_to_say'}, status=status.HTTP_404_NOT_FOUND)
        serializer = self.get_serializer(search, many=True)
        return Response(serializer.data)
    ...
    def male_only(self, request):
        ...
        if not male.exists():
            return Response({'message': 'no male children found'}, status=status.HTTP_404_NOT_FOUND)
