    ...
    if email and password:
        ...
        if user:
            if user.is_active:
                ...
                is_expired, token = (
                    ExpiringTokenAuthentication.validate_token_expiration(token)
                )
                if is_expired:
                ...
                return Response(
                    {
                        ...
                        "expires_in": ExpiringTokenAuthentication.expires_in(
                            token
                        ).total_seconds(),
                    }
                )
        ...
        return Response(
            {
                ...
                "expires_in_seconds": (
                    expires_in.total_seconds() if not is_expired else 0
                ),
